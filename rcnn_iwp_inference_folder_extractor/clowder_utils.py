...
def create_symlink_folder(host,key,dataset_id,folder_id,folder_name, folder_path= None, logger = None ):
    if os.getenv("MINIO_MOUNTED_PATH") == None:
        print("Minio mounted path not found")
        if logger is not None:
            logger.error("Minio mounted path not found")
        return
    ...
    minio_path = os.getenv("MINIO_MOUNTED_PATH")
    ...
    for file in files:
        ...
        file_id_dict[file_name] = file_id
    return folder_path, file_id_dict
